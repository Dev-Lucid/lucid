...
lucid::$mvc->controller('navigation')->render('view.patches-table');
...
$table = html::dataTable(_('model:patches'), 'patches-table', lucid::$mvc->model('patches'), 'app.php?action=view.patches-table');
...
$table->add(html::dataColumn(_('model:patches:identifier'), 'identifier', '45%', true));
$table->add(html::dataColumn(_('model:patches:applied_on_date'), 'applied_on_date', '45%', true));


# Add a column specifically for deleting rows.
$table->add(html::dataColumn('', null, '10%', false, function($data){
    return html::button(_('button:delete'), 'danger', "if(confirm('"._('button:confirm_delete')."')){ lucid.request('#!patches.delete|patch_id|".$data->patch_id."');}")->size('sm')->pull('right');
}));

# Enable searching this table based on some of the fields
$table->enableSearchFilter(['identifier']);

# Enable adding rows to the table. This simply links to the edit form, and passes the value 0 into the
# varialble $patch_id on the form.
$table->enableAddNewButton('#!view.patches-edit|patch_id|0', _('button:add_new'));
