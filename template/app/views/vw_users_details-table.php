...
$table->add(html::dataColumn(_('model:vw_users_details:email'), 'email', '12%', true));
$table->add(html::dataColumn(_('model:vw_users_details:password'), 'password', '12%', true));
$table->add(html::dataColumn(_('model:vw_users_details:first_name'), 'first_name', '12%', true));
$table->add(html::dataColumn(_('model:vw_users_details:last_name'), 'last_name', '12%', true));
$table->add(html::dataColumn(_('model:vw_users_details:org_id'), 'org_id', '12%', true));
$table->add(html::dataColumn(_('model:vw_users_details:organization_name'), 'organization_name', '12%', true));
$table->add(html::dataColumn(_('model:vw_users_details:role_id'), 'role_id', '12%', true));
$table->add(html::dataColumn(_('model:vw_users_details:role_name'), 'role_name', '12%', true));


# Add a column specifically for deleting rows.
$table->add(html::dataColumn('', null, '10%', false, function($data){
    return html::button(_('button:delete'), 'danger', "if(confirm('"._('button:confirm_delete')."')){ lucid.request('#!vw_users_details.delete|user_id|".$data->user_id."');}")->size('sm')->pull('right');
}));
...
$table->enableAddNewButton('#!view.vw_users_details-edit|user_id|0', _('button:add_new'));
