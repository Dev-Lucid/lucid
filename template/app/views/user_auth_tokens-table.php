...
lucid::$mvc->controller('navigation')->render('view.user_auth_tokens-table');
...
$table = html::dataTable(_('model:user_auth_tokens'), 'user_auth_tokens-table', lucid::$mvc->model('user_auth_tokens'), 'app.php?action=view.user_auth_tokens-table');
...
$table->add(html::dataColumn(_('model:user_auth_tokens:user_id'), 'user_id', '30%', true));
$table->add(html::dataColumn(_('model:user_auth_tokens:token'), 'token', '30%', true));
$table->add(html::dataColumn(_('model:user_auth_tokens:created_on'), 'created_on', '30%', true));
...
$table->enableSearchFilter(['token']);

# Enable adding rows to the table. This simply links to the edit form, and passes the value 0 into the
# varialble $token_id on the form.
$table->enableAddNewButton('#!view.user_auth_tokens-edit|token_id|0', _('button:add_new'));
