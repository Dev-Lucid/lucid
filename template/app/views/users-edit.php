...
lucid::$mvc->controller('navigation')->render('view.users-table', 'view.users-edit');

# Load the model. If $user_id == 0, then the model's ->create method will be called.
$data = lucid::$mvc->model('users', $user_id);
...
lucid::$mvc->controller('users')->ruleset()->send($form->name);

$org_id_options = lucid::$mvc->model('organizations')
    ->select('org_id', 'value')
    ->select('name', 'label')
    ->order_by_asc('name')
    ->find_array();
...
$card->block()->add([
    html::formGroup(_('model:users:org_id'), html::select('org_id', $data->org_id, $org_id_options)),
    html::formGroup(_('model:users:email'), html::input('text', 'email', $data->email)),
    html::formGroup(_('model:users:password'), html::input('text', 'password', $data->password)),
    html::formGroup(_('model:users:first_name'), html::input('text', 'first_name', $data->first_name)),
    html::formGroup(_('model:users:last_name'), html::input('text', 'last_name', $data->last_name)),
    html::formGroup(_('model:users:is_enabled'), html::input('checkbox', 'is_enabled', $data->is_enabled)),
    html::formGroup(_('model:users:last_login'), html::input('date', 'last_login', (new \DateTime($data->last_login))->format('Y-m-d H:i'))),
    html::formGroup(_('model:users:created_on'), html::input('date', 'created_on', (new \DateTime($data->created_on))->format('Y-m-d H:i'))),
    html::formGroup(_('model:users:force_password_change'), html::input('checkbox', 'force_password_change', $data->force_password_change)),
    html::formGroup(_('model:users:register_key'), html::input('text', 'register_key', $data->register_key)),
