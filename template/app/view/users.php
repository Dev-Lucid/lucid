...
class Users extends \App\View
{
    public function table()
    {
        ...
        lucid::factory()->view('navigation')->render('users.view.table');
        ...
        $table = html::dataTable(lucid::i18n()->translate('model:users'), 'users-table', $this->controller()->getList(), 'actions.php?action=users.view.table');
        ...
        $table->add(html::dataColumn(lucid::i18n()->translate('model:users:org_id'), 'org_id', '9%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:users:email'), 'email', '9%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:users:password'), 'password', '9%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:users:first_name'), 'first_name', '9%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:users:last_name'), 'last_name', '9%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:users:is_enabled'), 'is_enabled', '9%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:users:last_login'), 'last_login', '9%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:users:created_on'), 'created_on', '9%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:users:force_password_change'), 'force_password_change', '9%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:users:register_key'), 'register_key', '9%', true));


        # Add a column specifically for deleting rows.
        $table->add(html::dataColumn('', null, '10%', false, function($data){
            return html::button(lucid::i18n()->translate('button:delete'), 'danger', "if(confirm('".lucid::i18n()->translate('button:confirm_delete')."')){ lucid.request('#!users.controller.delete|user_id|".$data->user_id."');}")->size('sm')->pull('right');
        }));
        ...
        $table->enableAddNewButton('#!users.view.edit|user_id|0', lucid::i18n()->translate('button:add_new'));
    ...
    public function edit(int $user_id)
    {
        ...
        # permissions, use the lucid::$security->requirePermission() function.
        #lucid::permission()->requireLogin();
        # lucid::$security->requirePermission('users-select');

        # Set the title tag for the page. Optionally, you can also set the description or keywords meta tag
        # by calling lucid::$response->description() or lucid::$response->keywords()
        lucid::response()->title(lucid::i18n()->translate('branding:app_name').' - Users');

        # Render the navigation controller.
        lucid::factory()->view('navigation')->render('users.view.table', 'users.view.edit');
        ...
        $headerMsg = lucid::i18n()->translate('form:edit_'.(($data->user_id == 0)?'new':'existing'), [
            'type'=>'users',
            'name'=>$data->email,
        ]);
        ...
        $card->block()->add([
            html::formGroup(lucid::i18n()->translate('model:users:org_id'), html::select('org_id', $data->org_id, $org_id_options)),
            html::formGroup(lucid::i18n()->translate('model:users:email'), html::input('text', 'email', $data->email)),
            html::formGroup(lucid::i18n()->translate('model:users:password'), html::input('text', 'password', $data->password)),
            html::formGroup(lucid::i18n()->translate('model:users:first_name'), html::input('text', 'first_name', $data->first_name)),
            html::formGroup(lucid::i18n()->translate('model:users:last_name'), html::input('text', 'last_name', $data->last_name)),
            html::formGroup(lucid::i18n()->translate('model:users:is_enabled'), html::input('checkbox', 'is_enabled', $data->is_enabled)),
            html::formGroup(lucid::i18n()->translate('model:users:last_login'), html::input('date', 'last_login', (new \DateTime($data->last_login))->format('Y-m-d H:i'))),
            html::formGroup(lucid::i18n()->translate('model:users:created_on'), html::input('date', 'created_on', (new \DateTime($data->created_on))->format('Y-m-d H:i'))),
            html::formGroup(lucid::i18n()->translate('model:users:force_password_change'), html::input('checkbox', 'force_password_change', $data->force_password_change)),
            html::formGroup(lucid::i18n()->translate('model:users:register_key'), html::input('text', 'register_key', $data->register_key)),
