...
class Countries extends \App\View
{
    public function table()
    {
        ...
        lucid::factory()->view('navigation')->render('dashboard.view.admin', 'countries.view.table');
        ...
        $table = html::dataTable(lucid::i18n()->translate('model:countries'), 'countries-table', $this->controller()->getList(), 'actions.php?action=countries.view.table');
        ...
        $table->add(html::dataColumn(lucid::i18n()->translate('model:countries:alpha_3'), 'alpha_3', '23%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:countries:name'), 'name', '23%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:countries:common_name'), 'common_name', '23%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:countries:official_name'), 'official_name', '23%', true));


        # Add a column specifically for deleting rows.
        $table->add(html::dataColumn('', null, '10%', false, function($data){
            return html::button(lucid::i18n()->translate('button:delete'), 'danger', "if(confirm('".lucid::i18n()->translate('button:confirm_delete')."')){ lucid.request('#!countries.controller.delete|country_id|".$data->country_id."');}")->size('sm')->pull('right');
        }));
        ...
        $table->enableAddNewButton('#!countries.view.edit|country_id|0', lucid::i18n()->translate('button:add_new'));
    ...
    public function edit(string $country_id)
    {
        ...
        # Set the title tag for the page. Optionally, you can also set the description or keywords meta tag
        # by calling lucid::$response->description() or lucid::$response->keywords()
        lucid::response()->title(lucid::i18n()->translate('branding:app_name').' - Countries');

        # Render the navigation controller.
        lucid::factory()->view('navigation')->render('dashboard.view.admin', 'countries.view.table', 'countries.view.edit');

        # Load the model. If $country_id == 0, then the model's ->create method will be called.
        $data = $this->controller()->getOne($country_id);
        lucid::logger()->debug('country_id == '.$data->country_id);
        ...
        $headerMsg = lucid::i18n()->translate('form:edit_'.(($data->country_id == '')?'new':'existing'), [
            'type'=>'countries',
        ...
        $card->block()->add([
            html::formGroup(lucid::i18n()->translate('model:countries:alpha_3'), html::input('text', 'alpha_3', $data->alpha_3)),
            html::formGroup(lucid::i18n()->translate('model:countries:name'), html::input('text', 'name', $data->name)),
            html::formGroup(lucid::i18n()->translate('model:countries:common_name'), html::input('text', 'common_name', $data->common_name)),
            html::formGroup(lucid::i18n()->translate('model:countries:official_name'), html::input('text', 'official_name', $data->official_name)),
