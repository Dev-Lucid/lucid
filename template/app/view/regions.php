...
class Regions extends \App\View
{
    public function table()
    {
        ...
        lucid::factory()->view('navigation')->render('dashboard.view.admin', 'regions.view.table');
        ...
        $table = html::dataTable(lucid::i18n()->translate('model:regions'), 'regions-table', $this->controller()->getList(), 'actions.php?action=regions.view.table');
        ...
        $table->add(html::dataColumn(lucid::i18n()->translate('model:regions:country_id'), 'country_id', '15%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:regions:abbreviation'), 'abbreviation', '15%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:regions:name'), 'name', '15%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:regions:type'), 'type', '15%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:regions:parent'), 'parent', '15%', true));
        $table->add(html::dataColumn(lucid::i18n()->translate('model:regions:is_parent'), 'is_parent', '15%', true));


        # Add a column specifically for deleting rows.
        $table->add(html::dataColumn('', null, '10%', false, function($data){
            return html::button(lucid::i18n()->translate('button:delete'), 'danger', "if(confirm('".lucid::i18n()->translate('button:confirm_delete')."')){ lucid.request('#!regions.controller.delete|region_id|".$data->region_id."');}")->size('sm')->pull('right');
        }));
        ...
        $table->enableAddNewButton('#!regions.view.edit|region_id|0', lucid::i18n()->translate('button:add_new'));
        ...
        lucid::response()->replace('#main-rightcol', $table->render());
    }
    ...
    public function edit(string $region_id)
    {
        ...
        # permissions, use the lucid::$security->requirePermission() function.
        #lucid::permission()->requireLogin();
        # lucid::$security->requirePermission('regions-select');

        # Set the title tag for the page. Optionally, you can also set the description or keywords meta tag
        # by calling lucid::$response->description() or lucid::$response->keywords()
        lucid::response()->title(lucid::i18n()->translate('branding:app_name').' - Regions');

        # Render the navigation controller.
        lucid::factory()->view('navigation')->render('dashboard.view.admin', 'regions.view.table', 'regions.view.edit');
        ...
        $headerMsg = lucid::i18n()->translate('form:edit_'.(($data->region_id == '')?'new':'existing'), [
            'type'=>'regions',
            'name'=>$data->region_id,
        ]);
        ...
        $card->block()->add([
            html::formGroup(lucid::i18n()->translate('model:regions:country_id'), html::select('country_id', $data->country_id, $country_id_options)),
            html::formGroup(lucid::i18n()->translate('model:regions:abbreviation'), html::input('text', 'abbreviation', $data->abbreviation)),
            html::formGroup(lucid::i18n()->translate('model:regions:name'), html::input('text', 'name', $data->name)),
            html::formGroup(lucid::i18n()->translate('model:regions:type'), html::input('text', 'type', $data->type)),
            html::formGroup(lucid::i18n()->translate('model:regions:parent'), html::input('text', 'parent', $data->parent)),
            html::formGroup(lucid::i18n()->translate('model:regions:is_parent'), html::input('checkbox', 'is_parent', $data->is_parent)),
            html::input('hidden', 'region_id', $data->region_id),
        ]);
        $card->footer()->add(html::formButtons());

        $form->add($card);
        lucid::response()->replace('#main-rightcol', $form);
    }
}