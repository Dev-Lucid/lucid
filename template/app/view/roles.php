...
class Roles extends \App\View
{
    public function table()
    {
        ...
        lucid::$app->factory()->view('navigation')->render('dashboard.view.admin', 'roles.view.table');
        ...
        $table = html::dataTable(lucid::$app->i18n()->translate('model:roles'), 'roles-table', $this->controller()->getList(), 'actions.php?action=roles.view.table');
        ...
        $table->add(html::dataColumn(lucid::$app->i18n()->translate('model:roles:name'), 'name', '90%', true));


        # Add a column specifically for deleting rows.
        $table->add(html::dataColumn('', null, '10%', false, function($data){
            return html::button(lucid::$app->i18n()->translate('button:delete'), 'danger', "if(confirm('".lucid::$app->i18n()->translate('button:confirm_delete')."')){ lucid.request('#!roles.controller.delete|role_id|".$data->role_id."');}")->size('sm')->pull('right');
        }));
        ...
        $table->enableAddNewButton('#!roles.view.edit|role_id|0', lucid::$app->i18n()->translate('button:add_new'));
    ...
    public function edit(int $role_id)
    {
        ...
        # permissions, use the lucid::$app->$security->requirePermission() function.
        #lucid::$app->permission()->requireLogin();
        # lucid::$app->$security->requirePermission('roles-select');

        # Set the title tag for the page. Optionally, you can also set the description or keywords meta tag
        # by calling lucid::$app->$response->description() or lucid::$app->$response->keywords()
        lucid::$app->response()->title(lucid::$app->i18n()->translate('branding:app_name').' - Roles');
        ...
        $headerMsg = lucid::$app->i18n()->translate('form:edit_'.(($data->role_id == 0)?'new':'existing'), [
            'type'=>'roles',
            'name'=>$data->name,
        ]);
        ...
        $card->block()->add([
            html::formGroup(lucid::$app->i18n()->translate('model:roles:name'), html::input('text', 'name', $data->name)),
