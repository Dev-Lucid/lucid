...
class Roles extends \App\View
{
    public function table()
    {
        ...
        lucid::factory()->view('navigation')->render('dashboard.view.admin', 'roles.view.table');
        ...
        $table = html::dataTable(lucid::i18n()->translate('model:roles'), 'roles-table', $this->controller()->getList(), 'actions.php?action=roles.view.table');
        ...
        $table->renderer = function($data, string $column){
            return html::anchor('#!roles.view.edit|role_id|'.$data->role_id, $data->$column);
        };
        ...
        $table->add(html::dataColumn(lucid::i18n()->translate('model:roles:name'), 'name', '90%', true));


        # Add a column specifically for deleting rows.
        $table->add(html::dataColumn('', null, '10%', false, function($data){
            return html::button(lucid::i18n()->translate('button:delete'), 'danger', "if(confirm('".lucid::i18n()->translate('button:confirm_delete')."')){ lucid.request('#!roles.controller.delete|role_id|".$data->role_id."');}")->size('sm')->pull('right');
        }));
        ...
        $table->enableAddNewButton('#!roles.view.edit|role_id|0', lucid::i18n()->translate('button:add_new'));
    ...
    public function edit(int $role_id)
    {
        ...
        # Set the title tag for the page. Optionally, you can also set the description or keywords meta tag
        # by calling lucid::$response->description() or lucid::$response->keywords()
        lucid::response()->title(lucid::i18n()->translate('branding:app_name').' - Roles');

        # Render the navigation controller.
        lucid::factory()->view('navigation')->render('dashboard.view.admin', 'roles.view.table', 'roles.view.edit');
        ...
        $headerMsg = lucid::i18n()->translate('form:edit_'.(($data->role_id == 0)?'new':'existing'), [
            'type'=>'roles',
            'name'=>$data->name,
        ]);
        ...
        $card->block()->add([
            html::formGroup(lucid::i18n()->translate('model:roles:name'), html::input('text', 'name', $data->name)),
