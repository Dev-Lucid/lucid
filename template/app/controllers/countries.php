...
class ControllerCountries extends Controller
{
    ...
    public function ruleset(): Ruleset
    {
        return new Ruleset(__FILE__, __LINE__, [
            ['type'=>'lengthRange', 'label'=>_('model:countries:alpha_3'), 'field'=>'alpha_3', 'min'=>'2', 'max'=>'255', ],
            ['type'=>'lengthRange', 'label'=>_('model:countries:name'), 'field'=>'name', 'min'=>'2', 'max'=>'255', ],
            ['type'=>'lengthRange', 'label'=>_('model:countries:common_name'), 'field'=>'common_name', 'min'=>'2', 'max'=>'255', ],
            ['type'=>'lengthRange', 'label'=>_('model:countries:official_name'), 'field'=>'official_name', 'min'=>'2', 'max'=>'255', ],
        ]);
    }
    ...
    public function save(string $country_id, string $alpha_3, string $name, string $common_name, string $official_name, bool $do_redirect=true)
    {
        ...
        $data = lucid::$mvc->model('countries', $country_id, false);
    ...
    public function delete(int $country_id, bool $do_redirect=true)
    {
        ...
        lucid::$mvc->model('countries')->where('country_id', $country_id)->delete_many();
        if ($do_redirect === true) {
            lucid::redirect('countries-table');
        }
    }
