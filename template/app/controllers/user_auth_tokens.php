...
class ControllerUser_auth_tokens extends Controller
{
    ...
    {
        return new Ruleset([
            ['type'=>'length_range', 'label'=>_('model:user_auth_tokens:token'), 'field'=>'token', 'min'=>'2', 'max'=>'255', ],
        ]);
    }
    ...
    public function save(int $token_id, int $user_id, string $token, int $created_on, bool $do_redirect=true)
    {
        ...
        # This loads the table row that you are trying to update. If $token_id contains 0, then the model's
